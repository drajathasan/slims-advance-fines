...
if (isset($_POST['updateData'])) {
    ...
    // fines
    if (isset($_POST['advance_fines'])) {
        if (!isset($_POST['advance_fines']['allowed-users'])) {
          $_POST['advance_fines']['allowed-users'] = [];
        }
  
        Config::createOrUpdate('advance-fines', $_POST['advance_fines']);
      }
...
$form->addAnything('<h5>Pengaturan fitur Advance Fines</h5>', '');

$options = null;
$options[] = array('1', __('Enable'));
$options[] = array('0', __('Disable'));
$form->addSelectList('advance_fines[auto-paid]', 'Lengkapi Data Kredit?', $options, ((int)config('advance-fines.auto-paid', 0)),'class="form-control col-3"', 'Auto lunas tanpa kredit');

// user list
$user_query = $dbs->query('SELECT `user_id`, `realname` FROM `user` WHERE `user_id` != 1');
...
$form->addCheckBox('advance_fines[allowed-users]', 'Daftar Pengguna Advance-Fines', $user_options, config('advance-fines.allowed-users', []), 'Hanya pengguna terpilih saja yang bisa mengatur denda');
